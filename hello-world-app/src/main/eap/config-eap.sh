...
/opt/eap/bin/jboss-cli.sh '/subsystem=logging/syslog-handler=udpsyslog:add(server-address="rsyslog-server-udp.jboss-eap-test.svc.cluster.local",port=2514,facility=local-use-6,syslog-format=RFC3164,level=INFO,app-name=JBOSS_SAMPLE_APPLICATION,enabled=true,hostname=jboss-sample-application)' --connect
